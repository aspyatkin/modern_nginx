...
ngx_ct_tarball_path = ::File.join Chef::Config[:file_cache_path], ngx_ct_tarball
...
remote_file ngx_ct_tarball_path do
  ...
  checksum node[id][:ct][:tarball_checksum]
  mode 0644
end
...
execute "tar --no-same-owner -zxf #{ngx_ct_tarball} -C #{ngx_ct_src_dir} --strip-components 1" do
  cwd Chef::Config[:file_cache_path]
  creates ::File.join ngx_ct_src_dir, 'config'
end

node.run_state['nginx_configure_flags'] = node.run_state['nginx_configure_flags'] | ["--add-module=#{ngx_ct_src_dir}"]
